function [cash_1,house_1,gc_1d,ghouse2_1d,u,minhouse2] = my_long_loop3(t,Param1,Param2,Param3,grid,weig,minhouse2)
%% 重要：本函数是内置函数，不需要单独使用，也没有关键参数

%% Life-Cycle Consumption/Portfolio Choice Problem
% 这是一个没有股票的世界。用于模拟"之前没参与股票、当期也不打算支付one-time成本者"的最优化选择
% 其他设定与my_long_loop2一致
...
for i1=1:n
    greth(i1,1) = r+muh+grid(i1,1)*sigrh; %模拟的风险资产回报
end


for i1=1:nn
    %gret_sh(i1,1)=gret(ceil((i1)/n),1);
    gret_sh(i1,1)=greth(i1);
    gret_sh(i1,2)=weig(i1);
end
...
[i11_nn,~,i2_nh,i4_nc,i3_ncash,i1_nh]=ndgrid(i11,i5,i2,i4,i3,i1);
...
if t >= tr-tb
                     ...
                     house_1(ii,1) = ghouse2(i2_nh(ii),ii)*gret_sh(i11_nn(ii),1); % 下一期的房产就是本期房产乘以收益率
                     ...
                     if ghouse2(i2_nh(ii),ii)==ghouse(i1_nh(ii)) %假如没调整房产
                         ...
                         cash_1(ii,1) = r*sav +ret_fac ;
                         cash_1(ii,1) = max(min(cash_1(ii,1),gcash(ncash)),gcash(1));



else
                     house_1(ii,1) = ghouse2(i2_nh(ii),ii)*gret_sh(i11_nn(ii),1)/gyp; %下一期的房产就是本期房产乘以收益率
                     house_1(ii,1) = max(min(house_1(ii,1),ghouse(nh)),ghouse(1));
