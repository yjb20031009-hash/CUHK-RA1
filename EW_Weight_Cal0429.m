...
x = [ones(l,1) , mySample(:,3) , mySample(:,3).^2./1000 , mySample(:,4) , mySample(:,4).^2./1000, ...,
     mySample(:,5) , mySample(:,5).^2./1000 , mySample(:,6),  mySample(:,7)  , mySample(:,8)];
...
lamda4 = zeros(l,k);
y4 = mySample(:,10);
beta4 =(x_new'*x_new)\x_new'*y4;

for i=1:l
lamda4(i,:) = (Q*x_new(i,:)'*(y4(i,:)-x_new(i,:)*beta4)*sqrt(((l-1)/(l-k))))';
end
...
x = [ones(l,1) , mySample(:,3) , mySample(:,3).^2  , mySample(:,4) , mySample(:,4).^2 , ...,
     mySample(:,5) , mySample(:,5).^2  , mySample(:,6) ,  mySample(:,7)  , mySample(:,8)];
% x里包括 cons t t^2 cash cash^2 housing housing^2 Ipart treat*post treat post 
k = size(x,2);
lamda1 = zeros(l,k);
lamda2 = zeros(l,k);
...
x = [ones(l,1) , mySample(:,3) , mySample(:,3).^2./1000  , mySample(:,4) ,  mySample(:,4).^2./1000  , ...,
     mySample(:,5) , mySample(:,5).^2./1000  , mySample(:,6) , mySample(:,7), mySample(:,8), mySample(:,7).*mySample(:,8) ];
...
x4 = [ones(l,1) , mySample(:,3) , mySample(:,3).^2./1000  , mySample(:,4) ,  mySample(:,4).^2./1000  , ...,
...
beta4 = (x4'*x4)\x4'*y4;
for i=1:l
lamda4(i,:) = (Q*x4(i,:)'*(y4(i,:)-x4(i,:)*beta4)*sqrt(((l-1)/(l-k))))';
end
...
x3 = [ones(l,1) , mySample(:,3) , mySample(:,3).^2  , mySample(:,4) ,  mySample(:,4).^2  , ...,
     mySample(:,5) , mySample(:,5).^2  , mySample(:,13) , mySample(:,7), mySample(:,8), mySample(:,7).*mySample(:,8), mySample(:,13).*mySample(:,8)];
beta3 = (x3'*x3)\x3'*y3;
x4 = [ones(l,1) , mySample(:,3) , mySample(:,3).^2  , mySample(:,4) ,  mySample(:,4).^2  , ...,
     mySample(:,6) , mySample(:,7), mySample(:,8), mySample(:,7).*mySample(:,8) ];
beta4 = (x4'*x4)\x4'*y4;
...
beta4 = (x4_low'*x4_low)\x4_low'*y4_low;
