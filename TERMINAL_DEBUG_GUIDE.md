...
## 10) 全状态点、全期数前向模拟（策略回代体检）

如果你想“测试所有状态点、所有期数”的前向模拟，运行：

```bash
python forward_sim_all_states.py \
  --policy-mat fresh_jax_value_policy_gpu2.mat \
  --loop-mode loop2 \
  --out-csv forward_all_states_summary.csv \
  --save-dist forward_all_states_dist.npz
```

说明：
- 初始分布是 **所有 `(cash, house)` 状态点等权重**（确保每个状态都被覆盖）。
- 每期使用保存策略 `C1/A1/H1`（`loop2`）或 `C/A/H`（`loop1`）回代。
- 对每个状态按 `gret_sh` 冲击概率做转移，再映射回最近网格点，得到下一期分布。
- `forward_all_states_summary.csv` 会输出每期：
  - `mean_c/mean_a/mean_h_choice`
  - `share_participate/share_adjust_house`
  - `mean_cash_next/mean_house_next`
  - `mass_sum`（用于检查概率质量是否守恒）
